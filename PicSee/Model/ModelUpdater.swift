...
struct ModelUpdater {
    ...
    private static var updatedDrawingClassifier: HandskNN?
    /// The default Drawing Classifier model.
    private static var defaultDrawingClassifier: HandskNN {
        do {
            return try HandskNN(configuration: .init())
        } catch {
    ...
    private static let defaultModelURL = HandskNN.urlOfModelInThisBundle
    ...
    static func updateWith(trainingData: MLBatchProvider,
        ...
        HandskNN.updateModel(at: currentModelURL,
                             with: trainingData,
    ...
    private static func loadUpdatedModel() {
        ...
        guard let model = try? HandskNN(contentsOf: updatedModelURL) else {
            return
        }
